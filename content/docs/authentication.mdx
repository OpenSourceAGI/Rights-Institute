...
`trustedOrigins()` covers the apex, `www`, and localhost, plus the origin of
whatever `BETTER_AUTH_URL` / `NEXT_PUBLIC_APP_URL` is set to — so a preview
deployment isn't turned away by the CSRF origin check.
